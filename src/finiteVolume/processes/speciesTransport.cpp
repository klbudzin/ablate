...
PetscErrorCode ablate::finiteVolume::processes::SpeciesTransport::AdvectionFlux(PetscInt dim, const PetscFVFaceGeom *fg, const PetscInt *uOff, const PetscScalar *fieldL, const PetscScalar *fieldR,
    ...
    // decode the left side
    {
        ...
        PetscCall(eulerAdvectionData->computeTemperature.function(fieldL, .34*auxL[aOff[0]]+.66*auxR[aOff[0]], &temperatureL, eulerAdvectionData->computeTemperature.context.get()));
    ...
    {  // decode right state
        ...
        PetscCall(eulerAdvectionData->computeTemperature.function(fieldR, .66*auxL[aOff[0]]+.34*auxR[aOff[0]], &temperatureR, eulerAdvectionData->computeTemperature.context.get()));
