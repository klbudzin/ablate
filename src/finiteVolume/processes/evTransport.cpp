...
PetscErrorCode ablate::finiteVolume::processes::EVTransport::AdvectionFlux(PetscInt dim, const PetscFVFaceGeom *fg, const PetscInt *uOff, const PetscScalar *fieldL, const PetscScalar *fieldR,
    ...
    PetscReal pL;
    {
        ...
        PetscCall(eulerAdvectionData->computeTemperature.function(fieldL, .66*auxL[aOff[0]]+.34*auxR[aOff[0]], &temperatureL, eulerAdvectionData->computeTemperature.context.get()));
    ...
    {  // decode right state
        ...
        PetscCall(eulerAdvectionData->computeTemperature.function(fieldR, .34*auxL[aOff[0]]+.66*auxR[aOff[0]], &temperatureR, eulerAdvectionData->computeTemperature.context.get()));
...
void ablate::finiteVolume::processes::EVTransport::BoundExtraVariables(TS ts, ablate::solver::Solver &solver, const std::string &field) {
    ...
    auto eulerAccessor = solver.GetSubDomain().GetConstSolutionAccessor(CompressibleFlowFields::EULER_FIELD);
    auto densityEvAccessor = solver.GetSubDomain().GetSolutionAccessor(field);
    ...
    for (PetscInt c = cellRange.start; c < cellRange.end; ++c) {
        ...
        auto euler = eulerAccessor[cell];
        auto densityEv = densityEvAccessor[cell];
        // Only update if in the global vector
        if (euler) {
            // Get density
            const PetscScalar density = euler[CompressibleFlowFields::RHO];

            for (PetscInt sp = 0; sp < densityEvAccessor.GetField().numberComponents; sp++) {
                // Limit the bounds
