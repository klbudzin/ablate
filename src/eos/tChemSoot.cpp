...
// should be good
ablate::eos::TChemSoot::TChemSoot(std::filesystem::path mechanismFileIn, std::filesystem::path thermoFileIn)
    : EOS("TChemSoot"), mechanismFile(std::move(mechanismFileIn)), thermoFile(std::move(thermoFileIn)) {
    // setup/use Kokkos
    ...
    {  // manually compute reference enthalpy on the device
        ...
        stateHost.MassFractions()(0) = 0.0;  // TODO: What does this input represent? What do I put here?
...
PetscErrorCode ablate::eos::TChemSoot::SpecificHeatConstantPressureTemperatureFunction(const PetscReal *conserved, PetscReal temperature, PetscReal *cp, void *ctx) {
    ...
    ablate::eos::tChemSoot::SpecificHeatConstantPressure::runDeviceBatch(
        functionContext->policy, functionContext->stateDevice, functionContext->mixtureDevice, *functionContext->kineticsModelDataDevice);
...
PetscErrorCode ablate::eos::TChemSoot::SpecificHeatConstantVolumeTemperatureFunction(const PetscReal *conserved, PetscReal temperature, PetscReal *cv, void *ctx) {
    ...
    ablate::eos::tChemSoot::SpecificHeatConstantVolume::runDeviceBatch(
        functionContext->policy, functionContext->stateDevice, functionContext->mixtureDevice, *functionContext->kineticsModelDataDevice);
...
void ablate::eos::TChemSoot::FillWorkingVectorFromDensityMassFractions(double &density, double &temperature, const double *densityYi, const real_type_1d_view_host &totalStateVector,
                                                                       const int &totNumSpec) {
...
REGISTER(ablate::eos::EOS, ablate::eos::TChemSoot, "[TChemV2](https://github.com/sandialabs/TChem) ideal gas eos Agumented with a soot formation mechanism",
